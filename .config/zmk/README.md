...
[https://my.moergo.com/go60/#/layout/user/3e9cca22-62e9-430c-98dc-92e90fb60fdf]
## 方針
...
- **Sym 一枚で記号が全部打てる。** Shift との合成に頼らない — 当初は左手だけに記号を収めて残りを `J`(Shift) との合成で出す設計にしたが、使っている最中に `@` が無いことに気づいて破綻した (2026-09-03)
- **使わない位置は塞ぐ。** 未使用位置を `&trans` のままにすると Base が透けて混乱するので `&none` にする。**Base の数字行と両端列も同じ理由で潰した** — 当初は「レイヤーが体に入るまでの退路」として残していたが、不要になった。どこを透かしてどこを塞ぐかは下の「`&trans` と `&none` の使い分け」に一覧がある
