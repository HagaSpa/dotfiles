...
```
[MO(FN)][  死  ][   BS   ][英数/Ctrl] │ [Space][ かな/Cmd ][  死  ][ Opt ]
  1u      1.5u     1.5u     1.25u       1.25u      2u        1.5u    1u
  ✗     使わない  親指ホーム   内側        内側     親指ホーム 使わない   稀
```
...
- **一番外 (1u)** は親指では届かない。`MO(_FN)` (フラッシュ手順専用) と `Opt` を置いている。**`Opt` も理屈上は畳む動作**だが、`Opt+←/→` が `_NAV` にあるため素の Opt はほぼ使わず、頻度が桁違いなので据え置いている
