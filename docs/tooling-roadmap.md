...
### 4. mise tasks への統合（2026-07 実装済み）

mise 内蔵の[タスクランナー](https://mise.jdx.dev/tasks/)（依存関係・並列実行）に、`install.sh` の手続き連鎖（vim-plug → TPM → yazi plugins → karabiner build）を移す。（当初対象に含めていた `.config/zsh/tasks.sh` は 2026-07 に不使用のため削除済み）

- 既に mise を使っているため追加ツール不要。Make / just 相当を 1 ツールで済ませられる
- 「brew first / mise 格上げ」の既存方針（→ [CLAUDE.md](../CLAUDE.md)）とも整合する

**実装メモ（#156）**: タスクは `mise-tasks/` の file tasks として実装（`.mise.toml` は `~/.mise.toml` への symlink = global config のため、そこに書くと全プロジェクトを汚染する）。install.sh は bootstrap（Homebrew / Brewfile / mise runtimes）専用に縮小し、以降は `mise run setup` が依存 DAG で並列実行する。sources/outputs による up-to-date skip は、karabiner build の sync（`cmp -s || cp`）が内容不変時に mtime を進めない設計とタイムスタンプ比較が両立しないため不採用。
...
- ~~**nvim の lazy.nvim 化**~~: **2026-07 実装済み**。当初は「意図的に最小構成」として保留していたが、nvim 主力化の再トライアルでプラグインが 11 個まで増え、単一 `init.lua` が 393 行になった時点で分割した（#171 で lazy.nvim 導入 → その後 `lua/config` + `lua/plugins` に分割）。LazyVim への移行は検討して不採用（mason 前提の LSP スタックが PATH ベース運用と衝突し、core だけでプラグインが 11 → 28 に増えるため）
- **README のアーキテクチャ図（mermaid）・docs 索引の整備**: 実害がないため優先度低。上記 1〜4 で構成が変わってから書く方が手戻りがない

## 実施順序の目安

2（CI 近代化、実装済み）→ ~~3（lockfile、見送り）~~ → 4（mise tasks、実装済み）。当初の候補は解決済みで、残るのは保留中の 1（共通トークンの一元化）のみ。
