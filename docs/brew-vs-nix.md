...
## バージョン固定と Renovate 連携について（2026-07 追記）

「brew はバージョン固定の仕組みを持たない → Nix なら解決するか？ Nix にすれば Renovate が自動でライブラリ更新 PR を上げるか？」を確認した結果。**結論: Nix は固定できるようになるが、期待する形（ツール単位の自動 PR）にはならず、この判断を覆すほどの効果はない。**

- **Nix はバージョンを固定できる（brew との本質的な差）**: `flake.lock` に nixpkgs 全体の git コミットを記録し、そこから全ツールを解決する。brew が「常に最新・pin の概念なし」（[公式ドキュメント](https://docs.brew.sh/Brew-Bundle-and-Brewfile)、→ 本 PJ の #155 見送り）なのとは根本的に異なる
- **Renovate の Nix 対応はあるが粒度が違う**: [Renovate の nix manager](https://docs.renovatebot.com/modules/manager/nix/) は `flake.lock` を更新できるが、単位は **nixpkgs 全体の一括バンプ**でありパッケージ単位ではない。「ghostty だけ 1.3.1 に上げる」ような単独 PR にはならず「nixpkgs を丸ごと進める → 中の全ツールが一斉に動く」PR になる。加えて nix の [`lockFileMaintenance` はデフォルト無効で既知の癖もある](https://github.com/renovatebot/renovate/issues/29721)
- **一番 pin したい GUI ツールは Nix 化しても brew のまま**: ghostty / zed は macOS では nix-homebrew 経由 = 実体は brew cask なので、`flake.lock` には乗らない（本ドキュメント上部の GUI アプリの制約と同じ理由）
- **現に動いている Renovate は brew/Nix と無関係**: karabiner.ts（npm manager）と GitHub Actions（github-actions manager）の更新 PR はパッケージマネージャの選択と関係なく機能している

**この PJ での着地**: バージョン追跡は「brew ごと Nix 化」ではなく、(1) 版差が問題になるツールは mise へ格上げ（既存ポリシー）、(2) npm 依存（karabiner.ts）は Renovate の `rangeStrategy` 調整でレンジ内更新も PR 化、(3) GUI ツールの更新追跡は #155 どおり手動 or 見送り、で対応する。バージョン固定が Nix で可能であることは、Karabiner 非互換・mutable config・3 層化・単一マシンという見送り理由（上記）を覆さない。
...
- [Renovate nix manager](https://docs.renovatebot.com/modules/manager/nix/) / [nix lockFileMaintenance の挙動 issue](https://github.com/renovatebot/renovate/issues/29721)
