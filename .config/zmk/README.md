...
- 数字行 (行 1) と R5 行は `&trans` なので、Sym を抱えたまま数字も打てる
...
Num (レイヤー 2) が Sym (レイヤー 1) より上にある。両親指をホールドすると、ZMK は各キー位置で番号の高い方から `&trans` でない binding を探すので、**左手は Sym の記号 (Num 側が `&trans`)、右手は Num の数字**になる。

ただし **`=` `+` `-` は右手に移ったので、`x = 1 + 2` を同時ホールドで打ち切ることはできない**。記号と数字を混ぜるときは Sym と Num を持ち替える。全記号を 1 枚に載せることを優先した結果で、それ以前の「左手に記号・右手に数字」設計にあった利点は失われている。
